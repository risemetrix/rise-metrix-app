...
(function(){
  ...
  async function coachClaims(){
    await ensureClient();
    if(!meta.teamId) return [];
    const { data } = await sb.from("claims").select("id,player_id,act,pts,cat,note,created_at")
      .eq("team_id",meta.teamId).eq("status","pending").order("created_at");
    if(!data || !data.length) return [];
    const { data: ps } = await sb.from("players").select("id,name").eq("team_id",meta.teamId);
    return data.map(c=>({...c, player_name:((ps||[]).find(p=>p.id===c.player_id)||{}).name||"?"}));
  }
  async function resolveClaim(id,status){ await ensureClient();
  ...
  window.RMXCloud = {
    ...
    coachClaims, resolveClaim,
    joinTeam, athleteBoard:()=>athlete("board"),
    athleteClaim:(act,note)=>athlete("claim",{act,note}),
